 ...
 * DAM path handling.
 *
 * Lives in the shared package rather than the scraper because three places need
 * the same answer: the scraper when indexing, the API when resolving a lookup,
 * and the UI when the tool is published as a static snapshot with no API behind
 * it. Same reasoning as `parseSearchTerms` — one rule, one place. This function
 * has already had three separate bugs; a second copy would have had a fourth.
 */
